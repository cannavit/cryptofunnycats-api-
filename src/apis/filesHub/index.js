...
const { filesHub, schemafilesHub } = require('./model'); // new
...
router.get('/', async (req, res) => {
  ...
  const filesHubs = await filesHub.find();
...
router.post('/', async (req, res) => {
  ...
  await validateSchema(req.body, schemafilesHub);

  logger.warn(req.body);
  logger.info('save data: ' + JSON.stringify(req.body));

  const filesHub = await new filesHub(req.body);

  await filesHub.save();

  let filesHub2 = await filesHub.findOne(filesHub);

  logger.info(filesHub2);

  await res.send(filesHub2);
...
router.get('/filesHubs/:id', async (req, res) => {
  const filesHub = await filesHub.findOne({ _id: req.params.id });
...
router.get('/filesHubs/:id', async (req, res) => {
  try {
    ...
    res.status(404);
    res.send({ error: "filesHub doesn't exist!" });
  }
});
